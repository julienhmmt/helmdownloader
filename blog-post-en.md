...
- **Automatic image discovery.** No more hard-coded list. The tool runs `helm template`, recursively walks every rendered manifest, and extracts all image references — including the split `registry` / `repository` / `tag` / `digest` form many charts use.
- **Faster bundling.** Image pulling is *daemonless* via [go-containerregistry](https://github.com/google/go-containerregistry): **no Docker required**. Images download in parallel.
- **Shippable.** A static Go binary, compiled for Linux, macOS and Windows, on amd64 and arm64. Drop it in, run it.
- **No dependancies.** No need to have `docker` nor `podman` on your connected machine, *HelmDownloader* use a internal Golang lib.
...
The bundle it produces contains the chart, its `values.yaml`, each image as a tarball, an `images.txt` manifest mapping original references to their retagged versions, and a `load.sh` script that reloads and pushes everything to your registry on the other side.
...
The script reloads each image and pushes it to your registry. The chart itself is ready to deploy with `helm install`.
...
- **a ready-to-use bundle** with its own reload script.
