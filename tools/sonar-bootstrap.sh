...
# Bootstrap a local SonarQube container for helmdownloader.
...
ADMIN_USER="admin"
DEFAULT_PASS="admin"
TOKEN_NAME="helmdownloader-local"
...
PROJECTS="helmdownloader|helmdownloader (Go)"
...
if [ -f "$PASS_FILE" ]; then
    ...
    target_pass="HELMDOWNLOADER-$(openssl rand -base64 18 | LC_ALL=C tr -dc 'A-Za-z0-9')"
fi
...
main() {
    ...
    echo "Bootstrap complete. Project:"
    echo "$PROJECTS" | while IFS='|' read -r key name; do
        [ -z "$key" ] && continue
